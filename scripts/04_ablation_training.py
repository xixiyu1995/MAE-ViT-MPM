"""
Training hyperparameter ablation: dropout (0.1, 0.2, 0.3) and max_trans (0.2, 0.4, 0.6).
Optimal architecture per window size is fixed based on architecture ablation results:
- ws=8 : heads=4, depth=8, freeze_encoder=True, uses pretraining
- ws=16: heads=4, depth=4, freeze_encoder=True, uses pretraining
- ws=32: heads=8, depth=4, freeze_encoder=False, no pretraining (random init)
"""
...
MODEL_DIR = '../outputs/pretrained_models'
OUTPUT_DIR = '../outputs/ablation_results/training_ablation'
...
def run_training_ablation():
    ...
    for ws in WINDOW_SIZES:
        # Fixed architecture and pretraining strategy per window size
        if ws == 8:
            fixed_heads = 4
            fixed_depth = 8
            freeze_encoder = True
            pretrain_path = os.path.join(MODEL_DIR, f'mae_pretrained_ws{ws}_d{fixed_depth}_h{fixed_heads}.pth')
            if os.path.exists(pretrain_path):
                ...
                print(f"Pretrained model missing: {pretrain_path}, skipping ws={ws}")
                continue
        elif ws == 16:
            fixed_heads = 4
            fixed_depth = 4
            ...
            encoder_state = None   # No pretraining for ws=32

        print(f"\n========== Window size: {ws} | heads={fixed_heads}, depth={fixed_depth}, freeze={freeze_encoder} ==========")
        ...
        results_ws = []
        combinations = list(itertools.product(DROPOUT_VALUES, MAX_TRANS_VALUES))

        for (dropout, max_trans) in tqdm(combinations, desc=f"Training ws={ws}"):
            print(f"\n---> dropout={dropout}, max_trans={max_trans}")
            ...
            encoder_params = {
                ...
                'dropout': dropout    # Encoder dropout = current dropout value
            }
            ...
            for repeat in range(N_REPEATS):
                ...
                for dep_idx in range(n_deposits):
                    ...
                    probs, trues = finetune_fold(
                        ...
                        dropout_rate=dropout   # Classifier head dropout = same as encoder dropout
                    )
                    ...
                    detailed_records.append({
                        'window_size': ws, 'dropout': dropout, 'max_trans': max_trans,
            ...
            if detailed_records:
                ...
                detail_path = os.path.join(OUTPUT_DIR, f'details_ws{ws}_do{dropout}_mt{max_trans}.csv')
        ...
        if results_ws:
            ...
            out_path = os.path.join(OUTPUT_DIR, f'summary_training_ws{ws}.csv')
